...
Win_WarAnimaGuide = DlgTemplate:new({this = "Win_WarAnimaGuide"})

--初始化--

function Win_WarAnimaGuide:Init()
	self:RegisterEvent(WM_LBUTTONDOWN, self.OnLButtonDown)
	self:RegisterEvent(WM_MOUSEMOVE, self.OnMOUSEMOVE);
end

function Win_WarAnimaGuide:ShowDialog()
	GameApi.CovertTextArea(self.this, "Txt_TextArea", "欢迎来到战魂百科！")
...
function Win_WarAnimaGuide:OnLButtonDown(objName)
	for i = 1 , 6 do
		if objName == "Btn_" .. tostring(i) then
			...
			GameApi.CovertTextArea(self.this, "Txt_TextArea", WarAnimaGuide[i].text)
			--DlgApi.SetImageFile(self.this, "Img_Image", WarAnimaGuide[i].image, 1)
		end
	end
...
--鼠标移动
--------------------------------------------
function Win_WarAnimaGuide:OnMOUSEMOVE()
